...
int main(int argc, char **argv){
  ...
  if(file_bytes != MAP_ADDRESS){ /// ???//MAP_DAILED
  ...
  Elf64_Ehdr *ehdr = PTR_PLUS_BYTES(file_bytes,0); /// Sets a pointer to the ELF File Header
  ...
  ehdr -> e_ident[1] == 'E' &&
  ehdr -> e_ident[2] == 'L' &&
  ehdr -> e_ident[3] == 'F'; /// Checks identifying bytes for sequence {0x7f,'E','L','F'}
